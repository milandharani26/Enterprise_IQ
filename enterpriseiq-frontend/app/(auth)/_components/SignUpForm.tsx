...
export function SignUpForm() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!email) {
      ...
      if (!emailRegex.test(email)) {
        setEmailError("Please enter a valid, strictly formatted email address.");
        hasError = true;
      }
    }
    ...
    if (hasError) { toast.error("Please fix the validation errors."); return; }
  ...
  return (
    <motion.div variants={wrap} initial="hidden" animate="visible" className="w-full">
      ...
      <div className="relative rounded-2xl p-8 overflow-hidden bg-card/40 border border-border">
        ...
        <div className="absolute -top-16 -right-16 w-48 h-48 pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(52,211,153,0.12) 0%, rgba(52,211,153,0.04) 45%, transparent 70%)" }} />
        ...
        <motion.div variants={item} className="mb-7">
          ...
          <p className="text-sm mt-1 text-muted-foreground">Create your account on <span className="text-foreground font-semibold">EnterpriseIQ</span></p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-4">
          ...
          <motion.div variants={item} className="space-y-1.5">
            <Label htmlFor="email" className="text-xs font-medium text-muted-foreground/80">
              Email
            </Label>
          ...
          {/* Password */}
          <motion.div variants={item} className="space-y-1.5">
            <Label htmlFor="password" className="text-xs font-medium text-muted-foreground/80">
              Password
            </Label>
            <div className="relative">
          ...
          <AnimatePresence>
            {isError && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="p-3 text-xs text-red-500 dark:text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg text-center">
          ...
          <motion.div variants={item} className="flex items-center gap-3">
            ...
            <span className="text-[10px] uppercase tracking-widest font-medium text-muted-foreground/60">or continue with</span>
            <div className="h-px flex-1 bg-border/70" />
          </motion.div>
      ...
      {/* Sign in redirect */}
      <motion.p variants={item} className="text-center text-xs mt-5 text-muted-foreground">
